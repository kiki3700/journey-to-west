...
- 어댑터 패턴은 클래스 인터페이스를 사용자가 기대하는 다른 인터페이스로 변화하는 패턴으로, 호환성 없는 인터페이스 때문에 함께 동작할 수 없는 클래스들이 함께 작동하도록 해준다.
...
1. 클래스 어댑터: 객체를 래핑하지 않는다. 클라이언트와 서비스 양쪽에서 행동을 상속받는다. 오버라이딩 된 메서드에서 어댑테이션 된다.
