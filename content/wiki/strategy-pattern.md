...
- 소프트웨어 원칙 3가지
  1. 애플리케이션에서 달라지는 부분을 찾아내고 달리지지 않는 부분과 분리한다.
     1. 달라지는 부분은 캡슐화하여 행동을 위임한다.
     2. 변화하는 행동 부분은 클래스 집합을 만든다.
  2. 구현보다는 인터페이스에 맞춰서 프로그래밍한다.
     1. 실제 실행시 쓰이는 객체가 코드에 고정되지 않도록 상위 타입에 맞춰 프로그래밍해서 다형성을 활용한다.
  3. 상속보다는 구성을 활용한다.
     1. A에는 B가 있다. 두클래스를 합치는 방식
     2. 기존의 상속을 활용한 방식은 a는 b다.
