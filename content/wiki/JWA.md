...
알고리즘:
AES_128_CBC_HMAC_SHA_256: 128비트 키를 사용하는 AES-CBC + HMAC-SHA256
**AES_192_CBC_HMAC_SHA_죄송합니다. 방금 전에 답변이 중단된 것 같습니다. 이어서 설명을 마무리하겠습니다.
5.2. AES_CBC_HMAC_SHA2 Algorithms
알고리즘:
...
EC (Elliptic Curve): 타원 곡선 암호화 알고리즘에서 사용됩니다. 예를 들어, ES256 서명 알고리즘은 ECDSA를 기반으로 합니다.
...
주요 파라미터:
"crv" (Curve): 타원 곡선의 이름을 지정합니다. 예: P-256, P-384, P-521.
"x" (X Coordinate): 타원 곡선 공개 키의 X 좌표를 나타냅니다.
"y" (Y Coordinate): 타원 곡선 공개 키의 Y 좌표를 나타냅니다.
"d" (ECC Private Key): 타원 곡선 비공개 키를 나타내며, 이는 외부에 노출되지 않아야 합니다.
6.3. Parameters for RSA Keys
특징: RSA 키는 공개 키 암호화 및 서명에 사용됩니다. RSA 키는 큰 소수의 곱을 기반으로 하며, 보안성이 높지만, 타원 곡선 키에 비해 더 큰 키 크기를 필요로 합니다.
주요 파라미터:
"n" (Modulus): RSA 공개 키의 모듈러스 값으로, 키의 크기를 결정하는 중요한 요소입니다.
"e" (Exponent): RSA 공개 키의 지수 값입니다.
"d" (Private Exponent): RSA 비공개 키의 지수 값으로, 암호화된 데이터를 복호화하거나 서명을 생성하는 데 사용됩니다.
"p", "q": RSA 알고리즘의 프라임 팩터입니다.
"dp", "dq", "qi": 중국인의 나머지 정리를 활용한 최적화 값들입니다.
6.4. Parameters for Symmetric Keys
특징: 대칭 키는 암호화와 복호화에 동일한 키를 사용하는 방식입니다. 이는 성능이 빠르며, 주로 대량의 데이터를 암호화하는 데 사용됩니다.
주요 파라미터:
"k" (Key Value): 대칭 키의 실제 값을 나타내는 파라미터로, 보통 Base64 URL로 인코딩되어 저장 및 전송됩니다.
...
작동 원리:
키와 메시지 준비: 비밀 키와 메시지를 준비합니다.
키 패딩: 비밀 키를 해시 함수의 블록 크기와 맞게 패딩합니다.
내부 해시 생성: 비밀 키와 메시지를 해시 함수에 입력하여 내부 해시 값을 생성합니다.
외부 해시 생성: 내부 해시 결과와 키를 결합하여 최종 HMAC 값을 생성합니다.
주로 사용되는 부분:
...
작동 원리:
키 생성: 큰 소수 ppp와 qqq를 선택하고, n=p×qn = p \times qn=p×q를 계산합니다. 공개 키는 (n,e)(n, e)(n,e), 개인 키는 (n,d)(n, d)(n,d)로 설정합니다.
암호화: 공개 키로 메시지를 암호화합니다.
복호화: 개인 키로 암호문을 복호화합니다.
주로 사용되는 부분:
...
작동 원리:
키 생성: 개인 비밀 키로 공개 키를 생성합니다.
서명 생성: 메시지의 해시값을 서명하고 서명값을 생성합니다.
서명 검증: 공개 키와 서명값으로 서명의 유효성을 검증합니다.
주로 사용되는 부분:
...
작동 원리:
키 생성: 비밀 키를 사용하여 공개 키를 생성합니다.
서명 생성: 메시지 해시값과 랜덤 값을 사용하여 서명값을 생성합니다.
서명 검증: 공개 키와 서명값을 사용하여 서명의 유효성을 확인합니다.
주로 사용되는 부분:
...
작동 원리:
키 확장: 입력 키를 여러 라운드 키로 확장합니다.
암호화: 입력 블록을 암호화하여 기밀성을 보장합니다.
복호화: 암호화된 데이터를 원래 상태로 복원합니다.
주로 사용되는 부분:
...
작동 원리:
패딩: 데이터를 특정 형식으로 패딩하여 암호화합니다.
암호화: 패딩된 데이터를 공개 키로 암호화합니다.
복호화: 개인 키로 암호문을 복호화한 후 패딩을 제거합니다.
주로 사용되는 부분:
데이터 암호화: 데이터의 기밀성을 보장하며, 안전한 키 전송에 사용됩니다.
키 교환: 암호화된 키를 안전하게 교환하는 데 사용됩니다.
7. ECDH (Elliptic Curve Diffie-Hellman)
작동 원리:
키 생성: 비밀 개인 키와 공개 키를 생성합니다.
키 교환: 상대방의 공개 키를 사용하여 동일한 공유 비밀 키를 생성합니다.
주로 사용되는 부분:
