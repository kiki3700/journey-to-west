...
- 컴파일러 방식(Compiled Language): 소스코드를 한 번에 기계어로 변환하여 실행 파일을 생성하는 방식
  - 전처리, 컴파일, 어셈블러, 링킹 단계를 거쳐 빌드가 진행된다.
  - 이미 기계어로 번역되어 실행속도가 가장 빠르다는 장점이 있지만, 컴파일 시 특정 플랫폼(cpu, os)에 맞춰 빌드가 진행되기 때문에 플랫폼에 종속적이고, 빌드시 시간비용이 많이드는 단점이 있다.
...
- 하이브리드 방식(Hybrid Language): 컴파일러와 인터프리터 방식을 결합한 형태로, 보통 소스 코드 중간 코드(예: 바이트 코드)로 컴파일 한 후 그 중간 코드를 인터프리터가 실행하는 방식으로 동작한다. 두 가지 접근법의 장점을 결합하여, 효율적인 실행과 개발의 유연성을 모두 제공한다.
